class Api::ApiController < ApplicationController
  ...
  def next_image
    ...
    # Находим картинку в базе, чтобы CarrierWave отдал её правильный путевой объект
    image = Image.find(next_image_data[:image_id])

    render json: {
      new_image_index: next_image_data[:index],
      name: next_image_data[:name],
      file: { url: image.file.url }, # Теперь JS получит data.file.url и всё сработает!
  ...
  def prev_image
    ...
    # Находим картинку в базе, чтобы CarrierWave отдал её правильный путевой объект
    image = Image.find(prev_image_data[:image_id])
